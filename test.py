...
from lib.model.rpn.bbox_transform import bbox_transform_inv,bbox_transform_batch
...
if __name__ == "__main__":
    ...
    # from lib.model.aut_faster_rcnn.aut_vgg16_adv import vgg16
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=bs, shuffle=False, num_workers=0, pin_memory=True
    )
    ...
    for i in range(num_images):
        ...
        with torch.no_grad():
            ...
            im_cls_lb.resize_(data[-1].size()).copy_(data[-1])
        det_tic = time.time()
        (
            rois,
            cls_prob,
        ...
        # print("all_boxes shape:",len(all_boxes))
        # quit()
        misc_toc = time.time()
