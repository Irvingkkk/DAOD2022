...
class foggy_cityscape(imdb):
    ...
    def _get_voc_results_file_template_st(self,output_dir,pred_type):
        # VOCdevkit/results/VOC2007/Main/<comp_id>_det_test_aeroplane.txt
        filename = self._get_comp_id() + '_det_' + self._image_set + '_{:s}.txt'
        filedir = os.path.join(output_dir,pred_type)#(self._devkit_path, 'results', 'cityscape', 'Main')
        if not os.path.exists(filedir):
            os.makedirs(filedir)
        path = os.path.join(filedir, filename)
        return path

    def _write_voc_results_file_st(self, all_boxes,output_dir,imgname,pred_type):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print('Writing {} VOC results file'.format(cls))
            filename = self._get_voc_results_file_template_st(output_dir,pred_type).format(cls)
            imagename=[imgname]
            with open(filename, 'wt') as f:
                for im_ind, index in enumerate(imagename):
    ...
    def _do_python_eval_st(self, output_dir='output',ST=False):
        ...
        for i, cls in enumerate(self._classes):
            if cls == '__background__':
                continue
            pred_filename = self._get_voc_results_file_template_st(output_dir,pred_type='submodel_predictions').format(cls)
            rec, prec, ap = voc_eval_oneimg(
                pred_filename, annopath, imagesetfile, cls, ovthresh=0.5,
                use_07_metric=use_07_metric)
            aps += [ap]
            print('AP for {} = {:.4f}'.format(cls, ap))
            # with open(os.path.join(output_dir, cls + '_pr.pkl'), 'wb') as f:
        ...
        for ap in aps:
            print('{:.3f}'.format(ap))
        print('{:.3f}'.format(np.mean(aps)))
        if np.count_nonzero(aps)==0:
            criterion=0.0
        else: 
            criterion = np.sum(aps)/np.count_nonzero(aps)
        print("criterion:",criterion)
        return criterion # np.mean(aps)
        print('~~~~~~~~')
        print('')
        print('--------------------------------------------------------------')
        print('Results computed with the **unofficial** Python eval code.')
    ...
    def evaluate_detections_st(self, all_boxes, imgname,output_dir,ST=False):
        self._write_voc_results_file_st(all_boxes,output_dir,imgname=imgname,pred_type='submodel_predictions',)
        mAP=self._do_python_eval_st(output_dir,ST)
        if self.config['matlab_eval']:
            self._do_matlab_eval(output_dir)
        if self.config['cleanup']:
            for cls in self._classes:
