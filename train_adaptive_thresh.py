...
def checkdataloader(data_iter_s):
...
if __name__ == "__main__":
    with torch.autograd.set_detect_anomaly(True):
        ...
        exp_setup="test-AutoPL-ICRCCR-plema{}-0.01+2-st++warmup-s{}{}_SWAug_{}_mutual-{}{}_SWAug_{}_bs_{}_unsup_loss_w_{}_adv_loss_w_{}_EF_{}_aut_lr_{}_warmup_epoch_{}_mutual_epoch_{}_ema_weight_{}_lrdc_epoch{}".format(
                args.lambda_thresh,
            ...
            if epoch == warmup_epoch:
                ...
                for step in range(1, iters_per_epoch + 1):
                    ...
                    (
                        ...
                        _,
                        _
                    ) = teacher_fasterRCNN(im_data_weak, im_info,im_cls_lb, gt_boxes, num_boxes, test=True,target=True)
                    ...
                    # scores = scores.squeeze()  #[2, 300, 9] tensor
                    # pred_boxes = pred_boxes.squeeze() #[2, 300, 36]

                    #############################
                    ## prepare for adaptive thresh  ####
                    ##############################
                    if step == 1 and epoch == warmup_epoch+1:
                        ### threshold initialization ####
                        thresh_global=np.ones(imdb.num_classes)*0.5 #*0.8
                    else:
                        local_bank= [[] for i in range(1,imdb.num_classes+1)]
                        print("last_iter_thresh",thresh_global) 
                        for idx in range(args.batch_size):
                            for j in range(1,len(imdb.classes)):
                                score_map = scores[idx] #[300,9]
                                # print(torch.argmax(score_map,1))
                                classj_idx = torch.argmax(score_map,1) == j

                                if not len(torch.nonzero(classj_idx)) == 0:
                                    score_map_j = score_map[torch.nonzero(classj_idx)]
                                    class_j_mean=torch.mean(score_map_j[:,:,j])  
                                    # 取proposal被预测为概率最大的类别的probability的平均值作为模型对该类当前的学习程度
                                    local_bank[j].append(class_j_mean.item()) 
                        print("learning status now:\n",local_bank)
                        
                        for j in range(1,len(imdb.classes)):
                            if not len(local_bank[j]) == 0:
                                thresh_global[j]=args.lambda_thresh * thresh_global[j] + (1-args.lambda_thresh) * np.mean(local_bank[j])
                        print("this_iter_thresh",thresh_global)   
                        ...
                        if args.use_tfboard:
                            info = {
                                "bus_thresh": thresh_global[1],
                                "bicycle_thresh": thresh_global[2],
                                "car_thresh": thresh_global[3],
                                "motorcycle_thresh": thresh_global[4],
                                "person_thresh": thresh_global[5],
                                "rider_thresh": thresh_global[6],
                                "train_thresh": thresh_global[7],
                                "truck_thresh": thresh_global[8],
                            } 
                            logger.add_scalars(
                                "logs_s_{}/losses".format(args.session),
                                info,
                        ...
                        for j in range(1, len(imdb.classes)):
                            ...
                            if inds.numel() > 0:
                                ...
                                for i in range(np.minimum(30, cls_dets_numpy.shape[0])):   #Q:limit the number within 10??  A:per class pseudo_box is limited within 10
                                    ...
                                    if score > thresh_global[j]: #thresh:        # select proposals with confidence_score > 0.8 as pseudo label for distillation loss
