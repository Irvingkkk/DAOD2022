...
def select_reliable(imdbval_name,models,args):
    ...
    with torch.no_grad():
        for data in tbar:
            ...
            imgpath=data[-1] #['/space0/zhaofz/2022/faster-rcnn.pytorch/data/foggyCityscape/VOC2007/JPEGImages/target_munster_000089_000019_leftImg8bit_foggy_beta_0.02.jpg']
            ...
            with open("/space0/zhaofz/2022/faster-rcnn.pytorch/data/foggyCityscape/VOC2007/ST/test.txt",'w') as f:
                f.write(imgname)

            fake_gt = preds[-1]
            output_dir_root='/space0/zhaofz/2022/faster-rcnn.pytorch/data/foggyCityscape/VOC2007/ST'
    ...
    reliable_id_path = '/space0/zhaofz/2022/faster-rcnn.pytorch/data/foggyCityscape/VOC2007/ImageSets/Main'
...
def save_checkpoints(args,cfg,epoch,step):
...
if __name__ == "__main__":
    ...
    exp_setup="new-randaug-st++warmup-s{}{}_SWAug_{}_mutual-{}{}_SWAug_{}_bs_{}_unsup_loss_w_{}_adv_loss_w_{}_threshold_{}_aut_lr_{}_warmup_epoch_{}_mutual_epoch_{}_ema_weight_{}_nolrdc_{}".format(
        ...
        if epoch == warmup_epoch:
            ...
            for step in range(1, iters_per_epoch + 1):
                ...
                if step == iters_per_epoch:
                    ...
                    select_reliable(imdbval_name,warmup_checkpoints,args)
            ...
            for step in range(1, iters_per_epoch + 1):
                ...
                if step == iters_per_epoch // 2:
                    ...
                    data_iter_t = iter(t_dataloader)
                    save_checkpoints(args,cfg,epoch,step)
                if step % 1000 ==0:
                    save_checkpoints(args,cfg,epoch,step)
                ...
                if step == iters_per_epoch:
                    # imdb_target, _, target_dataloader = get_dataloader_aut(args.imdb_name_target, args, sequential=False, augment=args.mutual_use_SWAug)
                    imdbval_name="foggy_cityscape_trainval"
                    select_reliable(imdbval_name,mutual_checkpoints,args)


# def disp_interval(args,step,epoch):
                if step % args.disp_interval == 0:
                    end = time.time()
                    if step > 0:
                        loss_temp /= args.disp_interval
